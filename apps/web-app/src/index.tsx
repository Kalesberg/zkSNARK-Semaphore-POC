...
import DemoContract from "../../contracts/build/contracts/contracts/Demo.sol/Demo.json"
...
function App() {
    ...
    const listenEvents = useCallback(async () => {
      ...
      const events = await contract.queryFilter(contract.filters.GroupAdded())
    ...
    useEffect(() => {
        ;(async () => {
            ...
            if (accounts[0]) {
                ...
                setContract(new Contract(process.env.CONTRACT_ADDRESS!, DemoContract.abi, ethersProvider.getSigner()))
            }
    ...
    const createGroup = useCallback(async () => {
      if (signer && contract) {
          ...
          if (groupName) {
              setLoading.on()
              try {
                  const transaction = await contract.createGroup(formatBytes32String(groupName))
    ...
    const tryLogin = useCallback(async () => {
      if (activeUser && activeGroup2) {
        try {
          ...
          const { status } = await fetch(`${process.env.RELAY_URL}/add-signal`, {
              method: "POST",
